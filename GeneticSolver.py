...
class GeneticSolver:
    ...
    def solve(self, Y, delta, n_generations=-1):
        ...
        :param Y: 25x25 array that represents field in stopping condition
        ...
        :return: 25x25 array that represents the best start field found and associated fitness value
        """
    ...
    def _generate_population(self):
        ...
        :return: initial population as a list of 25x25 arrays
        """
        if type(self.initialization_strategy) is str:
            if self.initialization_strategy == 'uniform':
                initial_states = np.split(np.random.binomial(1, 0.5, (25 * self.population_size, 25)).astype('uint8'), self.population_size)
                ...
                return [life.make_move(np.random.binomial(1, prob, size=(25, 25)), moves=5) for prob in alive_probabilities]
            else:
            ...
            return [np.copy(self.initialization_strategy).reshape((25, 25)).astype('uint8') for i in range(self.population_size)]
    ...
    def mutate(cls, field):
        ...
        """
        a = np.random.binomial(1, 0.1, size=(25, 25)).astype('bool')
        field[a] += 1
        field[a] %= 2
        return field
    ...
    def fitness(cls, start_field, end_field, delta):
        ...
        return (candidate == end_field).sum() / 625
